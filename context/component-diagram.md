...
- **Preview Renderer** renders compiled pages to Canvas for in-viewport pages only, debounces zoom rasterization using global `preview_zoom_render_debounce_ms`, converts click positions into Typst page coordinates, dispatches `editor::FocusField` when WASM preview sync returns an Érgo focus target, and publishes the latest compiled outline, compiled resources, and displayed preview revision to the workspace sidebar.
