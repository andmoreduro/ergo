...
fn resolve_input_param(
    ...
    match &param.param_type {
        ...
        ParamType::AuthorList => {
            complex_params::resolve_author_list_param(&val, raw.unwrap_or(&serde_json::Value::Null), builder)
        }
        ParamType::AffiliationMap => {
            let letter_ids = ast.metadata.template_id == "umb-apa";
            complex_params::resolve_affiliation_map_param(&val, builder, letter_ids, "affiliations")
        }
        ParamType::DegreeMap => complex_params::resolve_degree_map_param(&val, builder),
    }
}
...
pub(super) fn rich_text_array_from_value(value: &serde_json::Value) -> Option<Vec<RichText>> {
