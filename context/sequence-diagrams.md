...
- The WASM preview session materializes only the files Typst compiles (`main.typ`, `lib.typ`, per-element `elements/*.typ`, `references.bib`). The `.ergproj/*.json` sidecars (document state, source maps, settings, dependency manifest, template) are written only by the backend session, which owns archive I/O. The field source map is worker-internal (preview caret/field sync) and is not serialized across the worker→main boundary; the main thread consumes only `source_revision`, `source_map`, and `dirty_resource_ids` from a sync.
