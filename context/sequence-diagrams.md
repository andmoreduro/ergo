...
- **Undo/redo:** apply the stored `inverseEvents` / `forwardEvents` locally and queue them for WASM `sync_events`; the resulting AST is mirrored to the backend by the sync barrier before the next save. Destructive inverses carry restore payloads (`RestoreElement`, `RestoreTableRow`, `RestoreTableColumn`).
