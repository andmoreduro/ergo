...
- **Tech9.6 (Preview Compile Contract) - WASM Canvas Preview:** As a developer, I want preview compilation in the WASM worker to retain a `PagedDocument`, source maps, and logical page metadata, with Canvas rasterization on demand, so that the live preview stays fast without depending on backend VFS SVG artifacts.
