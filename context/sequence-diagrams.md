...
- **Body virtualization:** top-level text blocks of the body use `content-visibility: auto` (setting `editor_virtualize_offscreen_blocks`), so off-screen blocks skip layout and paint while the DOM and ProseMirror model stay complete; the block(s) holding the selection are always laid out (`activeBlockPlugin` decoration) so caret placement, find and scroll-into-view measure real geometry. Node-view blocks (tables, figures, equations, diagrams) are not virtualized.
