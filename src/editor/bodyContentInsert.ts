...
import type { QuotePolicySpec } from "../bindings/QuotePolicySpec";
...
import { insertBodyInlineEquation, insertBodyInlineQuote } from "./prosemirror/bodyInsert";
import { shouldInsertQuoteAsBlock } from "./quote/quotePolicy";
...
export const tryBodyContentInsert = (
    ...
    if (elementType === "inlineEquation") {
        if (insertBodyInlineEquation("", deps.defaultEquationSyntax)) {
            return true;
        }
        return false;
    }
    ...
    if (
        elementType === "quote" &&
        !shouldInsertQuoteAsBlock(deps.quotePolicy) &&
        insertBodyInlineQuote("")
    ) {
        return true;
    }
    ...
    if (elementType === "heading") {
        const level =
            ...
            1;
        if (tryApplyHeadingLevelToCurrentBlock(level, deps.dispatch)) {
            return true;
        }
    }
    ...
    if (elementType === "quote") {
        if (shouldInsertQuoteAsBlock(deps.quotePolicy)) {
            dispatch({
                type: "ADD_QUOTE",
