...
/// Export a rectangular region of a page into a raster image (Érgo addition).
///
/// Renders only the half-open box `[x_min_pt, x_max_pt) × [y_min_pt, y_max_pt)`
/// (page-top-left-relative, in points; bleed is not rendered) at the given
/// resolution. The returned pixmap is `(x_max − x_min) × (y_max − y_min)` device
/// pixels. Content outside the box lands outside the smaller target pixmap, so
/// tiny-skia early-outs its per-pixel fills — raster cost and memory scale with
/// the visible region rather than the whole page. This is the high-zoom win,
/// where a page is several viewports tall/wide and only a sliver is on screen.
/// The page-relative container transform is preserved so gradients/patterns
/// align with a full `render`.
pub fn render_region(
    ...
    let pxw = (pixel_per_pt * (x1 - x0) as f32).round().max(1.0) as u32;
    let pxh = (pixel_per_pt * (y1 - y0) as f32).round().max(1.0) as u32;

    // World (pt) → device (px): scale by ppp, then shift the box's top-left to 0,0.
    let x_offset_px = pixel_per_pt * x0 as f32;
    let y_offset_px = pixel_per_pt * y0 as f32;
    let ts = sk::Transform::from_row(
        pixel_per_pt,
        0.0,
        0.0,
        pixel_per_pt,
