...
- **Preview Engine** wraps `DocumentSession`, `preview_pipeline`, dual `ErgoWorld` instances (main + resource previews with comemo), `PreviewSyncState`, and visible-region page rasterization. Region rasterization uses a vendored `ergo-typst-render` (`render_region`) that rasterizes only a rectangular slice (`x` and `y` bounds) of a page into straight-alpha RGBA, so cost scales with the on-screen area rather than the page.
- **Preview Pages** own DOM layout, viewport observation, visible-region computation (page rect intersected with the pane on both axes), canvas compositing, click coordinate conversion, and compile-driven page scroll. Each page is a `CanvasPreview` whose worker-rasterized region is uploaded as a transferable `ImageBitmap` and drawn (`drawImage`) onto a region-sized `<canvas>` positioned within the page surface; a white surface is the placeholder before paint and wherever the region does not cover. By default nothing off-screen (above/below or left/right) is rasterized; an advanced overscan setting pre-renders a margin.
...
- **Tauri API Client** imports IPC DTOs only from generated `src/bindings/`. Byte payloads (project files, packages, fonts, exports, imported assets) travel as raw IPC bodies decoded into typed-array views, so the UI thread never parses bytes as JSON; heavy commands execute on the backend's blocking pool, keeping the WebView responsive.
