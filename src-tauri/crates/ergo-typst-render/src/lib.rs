...
/// Export a rectangular region of a page into a raster image.
///
/// Renders only the half-open box `[x_min_pt, x_max_pt) × [y_min_pt, y_max_pt)`
/// (page-top-left-relative, in points) at the given resolution. The returned
/// pixmap is `(x_max − x_min) × (y_max − y_min)` device pixels. Content outside
/// the box lands outside the smaller target pixmap, so tiny-skia early-outs its
/// per-pixel fills — raster cost and memory scale with the visible region rather
/// than the whole page. This is the high-zoom win, where a page is several
/// viewports tall/wide and only a sliver is on screen. The page-relative container
/// transform is preserved so gradients/patterns align with a full `render`.
