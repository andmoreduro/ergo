...
- **Document State + History** (`DocumentContext`) stores local AST, queued events, undo entries `{ forwardEvent, inverseEvent, previousAst, nextAst }`, and focus state; WASM compile is the hot path, backend mirror is async for archives.
